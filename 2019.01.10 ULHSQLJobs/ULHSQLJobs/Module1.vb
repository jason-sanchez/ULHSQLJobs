...
Module Program
    ...
    Public Sub Main()
        ...
        Try
            ...
            dbstring = System.Configuration.ConfigurationManager.AppSettings(args(1).ToString())
            job = args(2).ToString()

            Dim jobname() As String = Split(job, ".")
            errorfilepath = errorfilepath & jobname(1) & "\"
            completefilePath = completefilePath & jobname(1) & "\"

            If Not Directory.Exists(errorfilepath) Then
                Directory.CreateDirectory(errorfilepath)
            End If

            If Not Directory.Exists(completefilePath) Then
            ...
            Using conn As New SqlConnection(dbstring)
                ...
                With objDBCommand
                    ...
                    logEvent("Success", job & "Job Complete - " & Date.Now, job, completefilePath)
        ...
        Catch ex As Exception
            ...
            logEvent("Error", Date.Now & " - " & ex.ToString(), job, errorfilepath)
    ...
    Private Sub RemoveOldLogs(ByVal filePath As String)
        ...
        Catch ex As Exception
            ex.ToString()
            logEvent("Error", Date.Now & " - " & ex.ToString(), "Log Removal Error", errorfilepath)
        End Try
